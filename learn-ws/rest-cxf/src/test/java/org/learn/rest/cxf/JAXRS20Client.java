...
public class JAXRS20Client {
	...
	public static void main(String[] args) {
		...
		List productList = ClientBuilder.newClient().register(jsonProvider).target(baseAddress).path("/products")
				.request(MediaType.APPLICATION_JSON).get(List.class);
		for (Object product : productList) {
			System.out.println(product);
		}

		List<Product> productList2 = ClientBuilder.newClient().register(jsonProvider).target(baseAddress)
